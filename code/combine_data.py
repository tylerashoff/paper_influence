...
keys, body_inds = np.unique(bodies[:, 0], return_index=True)

bodies = bodies[np.sort(body_inds), :]

inter, abs_mask, body_mask = np.intersect1d(
    list(abstracts[:, 0]), list(bodies[:, 0]), return_indices=True)

inter = reduce(np.intersect1d,
               (list(abstracts[:, 0]), list(bodies[:, 0]), list(df.pmcid)))
...
total_df = np.hstack(
    [cites, abs_ids, dates, journal, body_refs, abs_vecs, body_vecs])
