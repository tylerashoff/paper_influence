...
class embeddings():
    ...
    def plotting(self, X_fit, labs = None, embed='', stress=0, r2=0,
    ...
    def smacof(self, data, dim=3):
        ...
        smac,_,_ = manifold.smacof(dissimilarities = pairs,
                                   n_components=dim).fit_transform(data)
        return(smac)
    ...
    def n_month_k_mean_dists(self, vecs, dates, n=6, k=None):
        ...
        for date in dates:
            ...
            avg_dist = self.k_mean_dists(np.array([pairs[count-1, mask]]),
                                         k=k, calc_pairs = False)
...
meta = pd.read_csv('./code/meta_data_citations.csv')
...
for embedding in embeddings:
    print(embedding)
    emb_data, means, month_means, ball = emb.main(abs_vecs, dates,
                                                  embedding=embedding,
                                                  dim=5, k=5)

    data = np.hstack([np.array([pmcid]).T,
                      np.array([dates]).T,
                      np.array([journal]).T,
                      np.array([cites]).T,
                      np.array([refs]).T,
    ...
    emb_data, means, month_means, ball = emb.main(body_vecs, dates,
                                                  embedding=embedding,
                                                  dim=5, k=5)
