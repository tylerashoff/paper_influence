...
class txt_analysis():
    ...
    def find_files(self, pmcid):
        ...
        for path in paths:
            ...
            if str(pmcid) + '.xml.json' in os.listdir(path):
                ...
                json_attrs = np.append(pmcid, self.get_json_attrs(filename))
                pass
            pass
        return (json_attrs)
    ...
    def web_scrape(self, urls):
        ...
        for url in urls:
            ...
            # pull from another website, but unused in this study
                ...
                tree = html.fromstring(page.content)
                citations_str = tree.xpath(
                    '//*[@id="citing-articles-header"]/h2/text()')
                pass

            citation_count = [
                int(s) for s in citations_str[0].split() if s.isdigit()
            ] if len(citations_str) > 0 else []
    ...
    def process_docs(self, doc, minLength=2):
        ...
        corpum = nltk.Text([
            ...
            if (len(w) >= minLength and w.lower() not in stops and
    ...
    def docvec(self, docs, model_name=None, save=False):
        ...
        model = doc2vec.Doc2Vec(
            docs, window=300, min_count=1, workers=self.threads)
        if save:
    ...
    def docvec_update(self, docs, model_name, save=False):
        ...
        model.train(
            docs, total_examples=model.corpus_count, epochs=model.epochs)
        if save:
            model = deepcopy(model)
            model.save(model_name + '.bin')
            pass
    ...
    def docvec_files(self, dirs, model_name):
        ...
        for filename in filenames:
            ...
            if pmcids is None:
                ...
                self.docvec(json_file[:, 1], save=True, model_name=model_name)
                pass
            else:
                pmcids.extend(list(json_file[:, 0]))
                refs.extend(list(json_file[:, 2]))
                self.docvec_update(
                    json_file[:, 0], save=True, model_name=model_name)
                pass
            pass
        return (pmcids, refs)
    ...
    def infer_vectors(self,
        ...
        if dirs is not None:
            ...
            for filename in filenames:
                ...
                csv.writer(open(name, 'a', newline='')).writerows(
                    np.array(vecs))
                vecs = None
